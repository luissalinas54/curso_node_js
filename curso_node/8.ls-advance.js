...
async function ls (folder) {
  ...
  // ESTA SI ES UNA FUNCION EN PARALELO gracias a mat, que no espera que termine de hacerse una promes para empezar al otra
  const filePromises = files.map(async (file) => {
    ...
    return `${pc.bgMagenta(fileType)} ${pc.blue(file.padEnd(20))} ${pc.green(fileSize.toString().padStart(15))} ${pc.yellow(fileModified)}`;
  });
